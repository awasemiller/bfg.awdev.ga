...
if(!isset($_GET['games'])){
    $_GET['games'] = 'new-online';
	$request = 'glreleaseog';
}else if($_GET['games'] == 'new-online'){
	$request = 'glreleaseog';
}else if($_GET['games'] == 'top-online'){
	$request = 'glrankog';
}else{
...
Site_Parse::setTag('featureclass',($request=='glreleaseog') ? 'newgame' : 'topgame');

/**
 * set feature text
 */

Site_Parse::setTag('featuretext',Site_Language::display(($request=='glreleaseog') ? 'featured_online_newgame' : 'featured_online_topgame'));
...
$so = new Site_Online();
$so->processPageRequest($request,$genreInfo);
